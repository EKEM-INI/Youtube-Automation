...
async def _generate_voice_and_subtitles_async(text: str, voice_key: str, output_audio_path: str, output_ass_path: str):
    voice_name = VOICE_MAP.get(voice_key.lower(), "en-US-ChristopherNeural")
    
    communicate = edge_tts.Communicate(text, voice_name, rate="+5%")
    
    # Collect word-level boundaries and save audio
    ...
    async for chunk in communicate.stream():
        if chunk["type"] == "audio":
            ...
            word_boundaries.append({
                "text": chunk["text"],
                "offset": chunk["offset"] / 10_000_000, # 100ns units to seconds
    ...
    # Group words into short punchy 2-4 word phrases for TikTok / Shorts retention
    ass_dialogues = []
    chunk_size = 3
    ...
    if word_boundaries:
        for i in range(0, len(word_boundaries), chunk_size):
            ...
            # Highlight first word in yellow, rest in white
        ...
        # Fallback if boundaries not received: simple estimate
    ...
    # Build complete ASS Subtitle File with bold viral styling
